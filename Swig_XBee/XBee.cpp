...
{
	//usleep(time*1000);
  usleep(time*10); // sleep for 10uSeconds
  return ;
}
...
{
 struct timespec ts; 
   clock_gettime(_POSIX_MONOTONIC_CLOCK, &ts); // Clock monotonic keeps track of cycles
...
{
	/* Khol H
	 * Helper function returns the system time in milliseconds
	 * for Time stamps
	 */
	struct timeval ms;
	...
	seconds =(ms.tv_sec);
	useconds =(ms.tv_usec);
	return ((seconds)*1000L + (useconds)/1000);
}
#endif
...
{
	/*
	 * Default Constructor for XBee Response
	 *
	 */
	_frameDataPtr = NULL;
	_frameDataPtr = new uint8_t [1]; // Default allocation of 1 char.
...
XBeeResponse::XBeeResponse(int size)
{
	/*
	 * Explicitly Allocate memory by passing the size as an argument to the constructor
	 */
	_frameDataPtr = NULL;
	_frameDataPtr = new uint8_t[size];
	_apiId =0;
	_msbLength =0;
	_lsbLength =0;
	_checksum =0;
 ...
 XBeeResponse::~XBeeResponse()
	{
	 // check if _frameDataPtr is NULL before deleting to safeguard against errors
	   if(_frameDataPtr!=NULL) // check if _frameDataPtr is NULL before deleting to safeguard against errors
	   {
...
XBeeResponse::XBeeResponse(const XBeeResponse &Source)
	{

	/*
	 * Copy Constructor
	 * Dynamically Allocates Memory based on _frameLength
	 * Makes a Deep Copy
	 */
	_frameDataPtr = NULL;
	_frameLength =	Source._frameLength;
	_frameDataPtr =new uint8_t[_frameLength];

	for(int i =0; i<Source._frameLength; i++)
	{
		_frameDataPtr[i] = Source._frameDataPtr[i];
	}

	_apiId = 		Source._apiId;
	_msbLength = 	Source._msbLength;
	_lsbLength =	Source._lsbLength;
...
XBeeResponse& XBeeResponse::operator= ( const XBeeResponse &Source)
	{
	/*
		 * Assignment Operator
		 * Dynamically Allocates Memory based on _frameLength
		 * Makes a Deep Copy
		 */
		if(_frameDataPtr !=NULL)
			delete [] _frameDataPtr; // delete old _frameDataPtr data
		// Allocate some new Memory and copy contents
		_frameLength =	Source._frameLength;
		_frameDataPtr =new uint8_t[_frameLength];
		for(int i =0; i<Source._frameLength; i++)
		{
...
void XBeeResponse::setTimeStamp(double time)
	{
		timeStamp=time;
	}

double XBeeResponse::getTimeStamp()
{
	return timeStamp;
}
...
void XBeeResponse::getZBRxIoIdentifierResponse(XBeeResponse &response)
	{
		ZBRxIoIdentifierResponse* zb = static_cast<ZBRxIoIdentifierResponse*>(&response);
		// pass pointer array to subclass
		setCommon(response);
		zb->setFrameData(getFrameData());
...
FrameIdResponse::FrameIdResponse():XBeeResponse() {

}


uint8_t XBeeResponse::getFrameId()
{
...
AtCommandResponse::AtCommandResponse() : XBeeResponse(){
...
//Khol H
	/*updated setFrameData function to make a deep copy of the data contained in frameDataPtr
	 * if _frameDataPtr!=Null, delete the old pointer data and copy the new
	 * Allows for Dynamic Size Allocation of responses
	 */
	if(_frameDataPtr!=NULL)
		delete [] _frameDataPtr;

	_frameDataPtr =new uint8_t[_frameLength]; // new data ptr with length of data frame

	for(int i=0; i< _frameLength; i++)
	{
		_frameDataPtr[i] =frameDataPtr[i];
	}
...
XBee::XBee(){
	 ...
	 * Allocate 110 bytes for recieving all types of data frames
	 */
	 _response = XBeeResponse(MAX_FRAME_DATA_SIZE);
	_pos = 0;
	_escape = false;
	_checksumTotal = 0;
	_response.init();
	//_response.setFrameData(_response.getFrameData());
}
...
int  XBee::begin(int baud,const char* serialPort) {
	
	if(!Serial1.Open(serialPort, baud))
		return -1; 
	else 
		return 1; 
...
void XBee::resetSerial(int timeout)
{ 
	if(Serial1.IsOpened())
	{
		Serial1.Flush();
	...
	char c[1];
	while((GetTickCount()-t1) <timeout)
	{
		if(Serial1.ReadDataWaiting()> 0 )
		{
		Serial1.ReadData(c,1);
		//Serial1.Flush();
		}
		else 
			break;
...
void XBee::readPacketUntilAvailable() {
	...
	_response.setTimeStamp(getMillisecondTime());
...
bool XBee::readPacket(int timeout) {
    ...
    while (int((GetTickCount() - start)) < timeout) {
     	...
     	if (getResponse().isAvailable()) {
     		_response.setTimeStamp(getMillisecondTime()); // set the the packet was read
     		return true;
...
void XBee::readPacket() {
    ...
    while (Serial1.ReadDataWaiting()>0) {
...
ZBRxIoIdentifierResponse::ZBRxIoIdentifierResponse():XBeeResponse()
{
	/*
	 * Default Constructor
	 */
...
uint32_t ZBRxIoIdentifierResponse::getSN_MSB()
{ // the MSB of serial Number is located at bytes 0-3 of FrameData pointer
	return uint32_t((getFrameData()[0]<<24) + (getFrameData()[1]<<16) + (getFrameData()[2]<<8) +getFrameData()[3]);
}
uint32_t ZBRxIoIdentifierResponse::getSN_LSB()
{ // the LSB or Serial Number is located at bytes 4-7 of FrameData pointer
	return uint32_t((getFrameData()[4]<<24) + (getFrameData()[5]<<16) + (getFrameData()[6]<<8) + getFrameData()[7]);
}
uint16_t ZBRxIoIdentifierResponse::getRemoteAddress16()
{ // 16 bit address is stored at postion 9 and 10 of FrameData

	return uint16_t((getFrameData()[8]<<8) + getFrameData()[9]);
}

// the 64 bit and 16 bit address repeat again in this frame. the node ID is at position 21
string ZBRxIoIdentifierResponse ::getNodeID()
{// They Bytes starting at postion 21 are the Node ID of the XB
	// This is variable lenght  and so is terminated by a 0
	// thus we read each character until the 0 is reached,
	int position = 21;
	string NodeID;
	do{
		NodeID+=char(getFrameData()[position]);
		position++;
	}while(getFrameData()[position]!=0);
	return NodeID;
}


	uint16_t ZBRxIoIdentifierResponse ::getParentAddress()
	{
	  // returns the Parent Address of the Node that  sent the frame if it is an End Device
	  //since frames are variable length becase of the NODE ID's, we start at the back of the frame
	  return ((getFrameData()[getFrameDataLength()-8]<<8) + getFrameData()[getFrameDataLength()-7] );
				// returns the last 2 bytes of the frame
	}
	uint8_t ZBRxIoIdentifierResponse ::getDeviceType()
	{
		// Returns the Device Type
		// 0 = coord, 1 = Router  2= End Device
		//since frames are variable length becase of the NODE ID's, we start at the back of the frame
		return ( getFrameData()[getFrameDataLength()-6] );
		// returns the 6th from last byte of the frame

	}
	uint8_t ZBRxIoIdentifierResponse ::getSourceAction()
	{
		// returns the type of action that cased the NODE to create the Frame
		// 1 = Commission Button Response 2 = Node Join Response 3 = Rejoin
		//  since frames are variable length becase of the NODE ID's, we start at the back of the frame
		return (getFrameData()[getFrameDataLength()-5] );
		// returns the 5th from last of the Frame
	}
	uint16_t ZBRxIoIdentifierResponse ::getProfileID()
	{   // Profile ID
		//  since frames are variable length becase of the NODE ID's, we start at the back of the frame
		return uint16_t((getFrameData()[getFrameDataLength()-4]<<8) + (getFrameData()[getFrameDataLength()-3]) );
		// returns the 2nd to last 2 bytes of the frame
	}
	uint16_t ZBRxIoIdentifierResponse ::getManufactureID()
	{//  since frames are variable length becase of the NODE ID's, we start at the back of the frame
		return uint16_t((getFrameData()[getFrameDataLength()-2]<<8) + (getFrameData()[getFrameDataLength()-1]) );
		// returns the last 2 bytes of the frame
	}

 ...
 */
/* Modified XBee Library from Andrew Rap (2009) for Aduduino 
 Modified By Khol Haggerty Jan 2012
...
#ifdef _WIN32
#include "Serial.h"  // My windows Serial Wrapper 
#include <windows.h>
...
#ifdef __linux__
#include <time.h>
#include <unistd.h> // contains sleep and usleep
#include "CSerial.h" // My Linux Serial Wrapper
// Defualt Integer Types 
typedef unsigned int uint32_t;
typedef unsigned short uint16_t; 
typedef unsigned char uint8_t; 
// A Wrapper for GetTickCount for UNIX SYSTEMS 
void Sleep(unsigned int time); //  wrapper for Windows Sleep
unsigned long  GetTickCount(); // wrapper for gettickcount
...
#define	SUCCESS 0x06
#define COORDINATOR_FAILURE 0x2A
...
public:
	 ...
	 */
	/* Khol H
	 * Added a Default Constructor, Explicit Constructor, Destructor, Copy Constructor and assignment operator
	 * Multi-threaded need XBeeResponse Buffers
	 */
	XBeeResponse();
	 ...
	 */
	XBeeResponse(int size);



	virtual ~XBeeResponse(); // Destructor


	//Added Copy Constructor for Deep Copies!
	// Needed by Queue Class Buffer and Passing into Functions
	XBeeResponse(const XBeeResponse &Source);
	
	// Overloaded Assignment Operator returns Deep Copy as well
	XBeeResponse& operator= ( const XBeeResponse &Source);
	...
	/* Khol H
	 * Added this function for processing ZBRxIoIdentiferResponses
	 * only call in case of getApiId()==ZB_IO_IDENTIFIER_RESPONSE
	 */
	void getZBRxIoIdentifierResponse(XBeeResponse &id);
	...
	void setTimeStamp(double time); //Khol save the time stamp for Response
	double getTimeStamp(); //Khol H returns TimeStamp
...
private:
	...
	bool _complete;
	uint8_t _errorCode;
	double timeStamp; // Khol H Added Timestamp when response is read
	uint8_t _frameId;
};
...
class ZBTxStatusResponse : public XBeeResponse {
	public:
		ZBTxStatusResponse();
		virtual ~ZBTxStatusResponse(){};//delete []_frameDataPtr;};
...
public:
	 ...
	 */
	int begin(int baud,const char* serialPort);
	void getResponse(XBeeResponse &response);
	void Flush();

	void resetSerial(int timeout);
	/**
...
public:
	...
	virtual ~RemoteAtCommandRequest(){};
	/**
...
//Khol H
/* Added This class of response for ZB NODE IDENTIFIER RESPONSES
 * when A new node joins the network (or a sleeping  node wakes up) a ZB NODE IDENTIFIER RESPONSE is issued
 This class will parse data received from a Node Identifier Response
 and will be accessible through its member functions
*/
class ZBRxIoIdentifierResponse : public XBeeResponse {
public:
	ZBRxIoIdentifierResponse(); // default constructor
     virtual ~ZBRxIoIdentifierResponse(){}; // destructor
	uint16_t getRemoteAddress16(); // returns the 16-bit network address of the  Node

	uint32_t getSN_MSB(); // returns the MSB of 64-bit Serial Number
	uint32_t getSN_LSB(); // returns the LSB of 64-bit Serial Number

	string getNodeID(); // returns a string containing the Name of the node


	uint16_t getParentAddress(); // returns the Nodes'parent Network address (if known)
	uint8_t getDeviceType(); // returns the Nodes' device type (1=ROUTER,2=END_DEVICE)
	uint8_t getSourceAction(); // returns the source action that caused the Response (unused)
	uint16_t getProfileID(); //unused
	uint16_t getManufactureID(); //unused
